...
#     """
#     经验评估器
#     用于对不同类型（WHY, HOW, CHECK 等）的经验片段进行自动化评分和质量评估
#     """
#     def __init__(self):
#         # 评分标准
#          """初始化评估器，定义各类型片段的评分维度及权重"""
#         self.scoring_criteria = {
#             "WHY": {
#                 "goal_clarity": 0.4,     # 目标明确性权重
#                 "background": 0.2,        # 背景完整性权重
#                 "constraints": 0.2,       # 约束合理性权重
#                 "expected_outcome": 0.2   # 预期效果明确性权重
#             },
#             "HOW": {
#                 "completeness": 0.3,      # 步骤完整性权重
#                 "clarity": 0.3,           # 步骤清晰度权重
#                 "executability": 0.2,     # 可执行性权重
#                 "edge_cases": 0.2         # 边界情况处理权重
#             },
#             "CHECK": {
#                 "coverage": 0.4,          # 规则覆盖面权重
#                 "clarity": 0.3,           # 规则明确性权重
#                 "verifiability": 0.3      # 规则可验证性权重
#             }
#         }
        
#     def evaluate_fragment(self, fragment):
#         """
#         评估单个经验片段
#         Args:
#             fragment: 具有属性 frag_type 和 data 的片段对象
#         Returns:
#             包含 score、detail_scores、feedback 等信息的字典
#         """
...
#         # 根据片段类型选择评估方法
#         if frag_type == "WHY":
#             return self._evaluate_why_fragment(frag_data)
#         elif frag_type == "HOW":
#             return self._evaluate_how_fragment(frag_data)
#         elif frag_type == "CHECK":
#             return self._evaluate_check_fragment(frag_data)
#         else:
#             # 通用评估方法
#             return self._evaluate_generic_fragment(frag_data)
    
#     def _evaluate_why_fragment(self, data):
#         """评估WHY类型片段"""
#         if not data:
#             return {"score": 0.1, "feedback": ["数据为空"]}
        
#         score_parts = {}
#         feedback = []
        
#        # 1. 目标明确性：根据词数评估，10 词为理想长度
#         if "goal" in data and data["goal"]:
#             goal_len = len(data["goal"].split())
#             goal_clarity = min(1.0, goal_len / 10)  # 假设10个词是理想长度
#             score_parts["goal_clarity"] = goal_clarity
#             if goal_clarity < 0.5:
#                 feedback.append("目标描述过于简短")
#         else:
#             score_parts["goal_clarity"] = 0.0
#             feedback.append("缺少目标描述")
            
#         # 2. 背景完整性：20 词为理想长度
#         if "background" in data and data["background"]:
#             bg_len = len(data["background"].split())
#             bg_score = min(1.0, bg_len / 20)  # 假设20个词是理想长度
#             score_parts["background"] = bg_score
#             if bg_score < 0.5:
#                 feedback.append("背景描述不够详细")
#         else:
#             score_parts["background"] = 0.0
#             feedback.append("缺少背景描述")
            
#         # 3. 约束条件：理想数量 3 条
#         if "constraints" in data and isinstance(data["constraints"], list) and data["constraints"]:
#             constraints_count = len(data["constraints"])
#             constraints_score = min(1.0, constraints_count / 3)  # 假设3个约束是理想数量
#             score_parts["constraints"] = constraints_score
#         else:
#             score_parts["constraints"] = 0.0
#             feedback.append("缺少约束条件")
            
#         # 4. 预期效果：15 词为理想长度
#         if "expected_outcome" in data and data["expected_outcome"]:
#             outcome_len = len(data["expected_outcome"].split())
#             outcome_score = min(1.0, outcome_len / 15)
#             score_parts["expected_outcome"] = outcome_score
#             if outcome_score < 0.5:
#                 feedback.append("预期效果描述不够具体")
#         else:
#             score_parts["expected_outcome"] = 0.0
#             feedback.append("缺少预期效果描述")
            
#         # 计算加权总分
#         weights = self.scoring_criteria["WHY"]
#         total_score = sum(score_parts.get(k, 0) * weights.get(k, 0) for k in weights)
...
#     def _evaluate_how_fragment(self, data):
#         """评估HOW类型片段"""
#         if not data:
#             return {"score": 0.1, "feedback": ["数据为空"]}
...
#         if steps_count == 0:
#             return {"score": 0.1, "feedback": ["缺少操作步骤"]}
            
#         # 完整性评分
#         steps_completeness = min(1.0, steps_count / 5)  # 假设5步是标准
#         score_parts["completeness"] = steps_completeness
#         if steps_completeness < 0.6:
#             feedback.append(f"步骤数量较少({steps_count}步)")
...
#                 clarity_scores.append(min(1.0, words / 8))  # 假设8个词是标准
                
#         if clarity_scores:
#             avg_clarity = sum(clarity_scores) / len(clarity_scores)
#             score_parts["clarity"] = avg_clarity
#             if avg_clarity < 0.6:
#                 feedback.append("步骤描述不够详细")
#         else:
#             score_parts["clarity"] = 0.0
#             feedback.append("步骤缺少描述文本")
            
#         # 可执行性和边界情况默认评分
#         score_parts["executability"] = 0.7  # 默认相对较高
#         score_parts["edge_cases"] = 0.5   # 默认中等
        
#         # 计算加权总分
#         weights = self.scoring_criteria["HOW"]
#         total_score = sum(score_parts.get(k, 0) * weights.get(k, 0) for k in weights)
...
#     def _evaluate_check_fragment(self, data):
#         """评估CHECK类型片段"""
#         if not data:
#             return {"score": 0.1, "feedback": ["数据为空"]}
...
#         if rules_count == 0:
#             return {"score": 0.1, "feedback": ["缺少校验规则"]}
            
#         # 覆盖面评分
#         coverage_score = min(1.0, rules_count / 3)  # 假设3条规则是标准
#         score_parts["coverage"] = coverage_score
#         if coverage_score < 0.7:
#             feedback.append(f"规则数量较少({rules_count}条)")
            
#         # 规则明确性和可验证性默认评分
#         score_parts["clarity"] = 0.7     # 默认相对较高
#         score_parts["verifiability"] = 0.6  # 默认中等偏上
        
#         # 计算加权总分
#         weights = self.scoring_criteria["CHECK"]
#         total_score = sum(score_parts.get(k, 0) * weights.get(k, 0) for k in weights)
...
#     def _evaluate_generic_fragment(self, data):
#         """评估通用类型片段"""
#         if not data:
#             return {"score": 0.1, "feedback": ["数据为空"]}
...
#         completeness = non_empty_fields / fields_count if fields_count > 0 else 0
#         score = completeness * 0.7 + 0.1  # 基础分0.1，最高0.8
        
#         feedback = []
#         if completeness < 0.5:
#             feedback.append(f"数据完整度低({completeness*100:.0f}%)")
...
#     def evaluate_experience_pack(self, exp_pack):
#         """
#         评估整个经验包
...
#             # 按片段类型权重累加分数
#             weight = type_weights.get(fragment.frag_type, 0.2)
...
#                     suggestions.append(f"{fragment.frag_type}: {fb}")
...
#         # 检查是否缺少关键片段类型
#         for key_type in type_weights:
#             if key_type not in fragment_results:
#                 suggestions.append(f"缺少{key_type}类型片段")
        
#         return {
#             "overall_score": overall_score,
#             "fragment_scores": fragment_results,
#             "suggestions": suggestions[:5],  # 最多返回5条建议
#             "completeness": len(fragment_results) / len(type_weights)
#         }
...
#     result = evaluator.evaluate_fragment(why_frag)
#     print("WHY片段评分:", result["score"])
#     print("反馈:", result.get("feedback", []))

# experienceagent/fragment_scorer.py
# experienceagent/fragment_scorer.py
"""
经验片段评分器
